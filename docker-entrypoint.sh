...
SYNC_CONFIGS=${SYNC_STRATEGY_CONFIGS_FROM_REPO:-0}
if [ "$SYNC_CONFIGS" = "1" ]; then
  cp -f /app/config.defaults/*.json strategies/configs/ 2>/dev/null || true
  echo "Synced strategy configs from repo (image build)."
